...
from transformers import AutoTokenizer, CLIPTextModel, AutoProcessor, CLIPVisionModel, CLIPConfig, CLIPProcessor
...
parser.add_argument("--root_path", type=str, help="数据集根目录", default="C:\\Users\\lyq\\DataSet\\FakeNews\\gossipcop")
parser.add_argument("--model_dir", type=str, help="模型目录", default='C:\\Users\\lyq\\Model\\LongCLIP-GmP-ViT-L-14')
...
parser.add_argument("--max_length", type=int, help="输入文本最大长度",default=248)
...
class CLIP(nn.Module):
    ...
    def __init__(self, model_dir, *args, **kwargs):
        super().__init__()
        config = CLIPConfig.from_pretrained(model_dir)
        config.text_config.max_position_embeddings = kwargs['max_length']
        self.text_model = CLIPTextModel.from_pretrained(model_dir, config=config.text_config)
        self.image_model = CLIPVisionModel.from_pretrained(model_dir)
